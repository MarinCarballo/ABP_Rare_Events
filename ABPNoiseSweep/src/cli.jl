...
function abp_apply_env_overrides!(cfg::ABPNoiseSweepConfig)
    ...
    haskey(ENV, "ABP_PATH_OBSERVATION_STRIDE") && (cfg.path_observation_stride = parse(Int, ENV["ABP_PATH_OBSERVATION_STRIDE"]))
    haskey(ENV, "ABP_PATH_TIME_STRIDE") && (cfg.path_time_stride = parse(Int, ENV["ABP_PATH_TIME_STRIDE"]))
    haskey(ENV, "ABP_PATH_FILTER_X_MIN") && (cfg.path_filter_x_min = parse(Float64, ENV["ABP_PATH_FILTER_X_MIN"]))

    haskey(ENV, "ABP_SAVE_MOVIE_PATHS") && (cfg.save_movie_paths = abp_env_bool("ABP_SAVE_MOVIE_PATHS", cfg.save_movie_paths))
    haskey(ENV, "ABP_MOVIE_CHAIN_ID") && (cfg.movie_chain_id = parse(Int, ENV["ABP_MOVIE_CHAIN_ID"]))
    haskey(ENV, "ABP_MOVIE_STRIDE") && (cfg.movie_stride = parse(Int, ENV["ABP_MOVIE_STRIDE"]))
    haskey(ENV, "ABP_MOVIE_PATH_TIME_THIN") && (cfg.movie_path_time_thin = parse(Int, ENV["ABP_MOVIE_PATH_TIME_THIN"]))
    haskey(ENV, "ABP_MOVIE_MAX_TRAJECTORIES") && (cfg.movie_max_trajectories = parse(Int, ENV["ABP_MOVIE_MAX_TRAJECTORIES"]))
    haskey(ENV, "ABP_MOVIE_ENDPOINT_X_MIN") && (cfg.movie_endpoint_x_min = parse(Float64, ENV["ABP_MOVIE_ENDPOINT_X_MIN"]))
    haskey(ENV, "ABP_MOVIE_ENDPOINT_X_MAX") && (cfg.movie_endpoint_x_max = parse(Float64, ENV["ABP_MOVIE_ENDPOINT_X_MAX"]))

    haskey(ENV, "ABP_ROUNDTRIP_TARGET") && (cfg.roundtrip_target = parse(Int, ENV["ABP_ROUNDTRIP_TARGET"]))
    haskey(ENV, "ABP_ROUNDTRIP_AVG_TARGET_FRACTION") && (cfg.roundtrip_avg_target_fraction = parse(Float64, ENV["ABP_ROUNDTRIP_AVG_TARGET_FRACTION"]))
    haskey(ENV, "ABP_ROUNDTRIP_CONVERGENCE_HITS") && (cfg.roundtrip_convergence_hits = parse(Int, ENV["ABP_ROUNDTRIP_CONVERGENCE_HITS"]))
...
function abp_print_config(cfg::ABPNoiseSweepConfig)
    ...
    println("  path_observation_stride = ", cfg.path_observation_stride)
    println("  path_time_stride = ", cfg.path_time_stride)
    println("  path_filter_x_min = ", cfg.path_filter_x_min)
    println("  save_movie_paths = ", cfg.save_movie_paths)
    if cfg.save_movie_paths
        println("  movie_chain_id = ", cfg.movie_chain_id)
        println("  movie_stride = ", cfg.movie_stride)
        println("  movie_path_time_thin = ", cfg.movie_path_time_thin)
        println("  movie_max_trajectories = ", cfg.movie_max_trajectories)
        println("  movie endpoint window = [", cfg.movie_endpoint_x_min, ", ", cfg.movie_endpoint_x_max, ")")
    end
    println("  roundtrip_target = ", cfg.roundtrip_target)
    println("  roundtrip_avg_target_fraction = ", cfg.roundtrip_avg_target_fraction)
    println("  roundtrip_convergence_hits = ", cfg.roundtrip_convergence_hits)
